...
const ValueText = styled.span<{ $shaded?: boolean }>(
  ({ $shaded, ...props }) => ({
    wordWrap: "break-word",
...
function truncateTextFromMiddle(text: string, length = 16): string {
  if (text) {
...
function KeyValuePair({
  ...
  return (
    <KeyValueContainer $shaded={shaded}>
      ...
      <ValueText $shaded={shaded}>
        {truncate ? truncateTextFromMiddle(value) : value}
      </ValueText>
    </KeyValueContainer>
  );
}
