...
export function InscriptionsList({
  ...
  inscriptions: Array<InscriptionsI>;
  handleLoadMore: () => void;
  ...
  return (
    <>
      {inscriptions && (
        <SectionContainer data-testid="inscriptions-list">
          <label>Results</label>
          {inscriptions?.map(({ id }) => (
            <div
              onClick={() => navigate(`/detail/${walletAddress}/${id}`)}
              key={id}
            >
              <InscriptionRow>
                <span>Inscription {trimToChars(id)}</span>
